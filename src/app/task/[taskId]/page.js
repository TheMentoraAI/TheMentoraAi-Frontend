...
export default function TaskPage({ params }) {
  ...
  useEffect(() => {
    async function fetchTask() {
      ...
      if (fetchedIds.current.has(taskId)) return;
      fetchedIds.current.add(taskId);
          ...
          <header className={styles.header}>
            ...
            <div className={styles.taskInfo}>
              ...
              <span className={styles.taskStatus}>
                {isCompleted ? (
                  <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight: '4px'}}><polyline points="20 6 9 17 4 12"/></svg> Completed</>
                ) : (
                  <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight: '4px'}}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> Active Task</>
                )}
              </span>
            </div>
          </header>
